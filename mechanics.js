...
(function () {
  ...
  if (!GAME.entities.pickups) GAME.entities.pickups = [];
  ...
  function spawnPickups() {
    ...
    for (var i = 0; i < PICKUP_COUNT; i++) {
      ...
      pickup.position.set((Math.random() * 2 - 1) * 20, 0.35, (Math.random() * 2 - 1) * 20);
  ...
  if (GAME.entities.pickups.length === 0) spawnPickups();

  function updatePlayer(deltaTime) {
    if (!player || GAME.state.isPaused || GAME.state.isGameOver) return;
    player.yaw = yaw; player.pitch = pitch; player.rotation.y = yaw;
    ...
    if (moveX !== 0 || moveZ !== 0) {
      ...
      var sinYaw = Math.sin(yaw), cosYaw = Math.cos(yaw);
  ...
  function checkPickups() {
    ...
    for (var i = GAME.entities.pickups.length - 1; i >= 0; i--) {
      ...
      if (!pickup) { GAME.entities.pickups.splice(i, 1); continue; }
      ...
      if (distSq <= PICKUP_RADIUS_SQ) {
        ...
        if (GAME.hud && typeof GAME.hud.showStatus === 'function') GAME.hud.showStatus('+10 points', 1000);
      }
    }
  }
  ...
  if (typeof GAME.registerUpdate === 'function') {
    GAME.registerUpdate(function (deltaTime) { updatePlayer(deltaTime); checkPickups(); });
  }
