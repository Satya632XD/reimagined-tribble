...
(function () {
  ...
  if (!window.GAME) { console.error('[visuals.js] GAME global not found.'); return; }
  if (!window.THREE) { console.error('[visuals.js] THREE global not found.'); return; }
  ...
  const upgradedGroundMaterial = new THREE.MeshStandardMaterial({
    map: groundTexture, roughness: 0.7, metalness: 0.15
  });
  ...
  for (let i = 0; i < particleCount; i++) {
    ...
    driftSpeeds[i] = 0.6 + Math.random() * 1.4;
  }
  ...
  const particleMaterial = new THREE.PointsMaterial({
    size: 0.4, map: particleTexture, blending: THREE.AdditiveBlending,
  ...
  GAME.registerUpdate(() => {
    ...
    if (GAME.entities && GAME.entities.player) {
      ...
      const professionalPlayerMaterial = new THREE.MeshStandardMaterial({
        color: 0x2563eb, roughness: 0.25, metalness: 0.75
      });
